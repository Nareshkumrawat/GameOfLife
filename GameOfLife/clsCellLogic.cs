...
namespace GameOfLife
{
    ...
    static class clsCellLogic
    {
        ...
        public static CellTypeEnum GetCellType(clsGrid grid, CoOrdinates coOrdinates)
        {
            if ((coOrdinates.X < -1 || coOrdinates.X > grid.RowCount) || (coOrdinates.Y < -1 || coOrdinates.Y > grid.ColumnCount))
            {
                throw new ArgumentOutOfRangeException("Must be greater than or equal to minus one and less than or equal to Row count");
            }
